...
const SaveTeam = () => {
  ...
  const substitute = async () => {
    try {
      ...
      const res = await axios.post(
        `${process.env.BACKEND_URL}/substitute/player`,
        {
          toSubstituteId: subOut,
          fromSubstituteId: subIn,
        },
        {
          headers: {
            Authorization: `Bearer ${session?.data.data.token}`,
      ...
      setMessage("Substitute successful");
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(delayedLogic, 7000);
      ...
      {substitution && (
        <div className="fixed inset-0 z-[150] overflow-y-auto bg-[#000000]/50 ">
          <div className="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="relative inline-block  pt-5 pb-4 overflow-hidden text-center align-bottom transition-all transform bg-white rounded-2xl shadow-xl  sm:my-8 w-full sm:max-w-2xl sm:p-6 md:p-8 sm:align-middle">
              <div className="flex items-end justify-end">
                <button
                  onClick={(e) => {
                    e.preventDefault();
                  ...
                  <div className="flex justify-between  mt-10 sm:mx-auto mx-3 sm:w-3/4">
                    {teams.subs.map((item: Players, player_id: number) => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          setSubIn(item.id as any);
                          substitute();
                        }}
                        className=" rounded mt-2 mx-auto  hover:scale-110  transition transform duration-500 cursor-pointer"
                      >
                        <div className="-my-[1.5rem] mx-auto relative">
                          <img
                            className="rounded-full bg-[#fff] -translate-y-1/2 transform object-cover object-center mx-auto w-[3rem]"
